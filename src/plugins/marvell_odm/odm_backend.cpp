...
nixlOdmEngine::prepXfer(const nixl_xfer_op_t &operation,
    ...
    if (dram_to_dram) {
        ...
        req->direction = (operation == NIXL_READ) ? ODM_DIR_TO_GPU : ODM_DIR_FROM_GPU;
        req->gpu_id = 0;
        NIXL_DEBUG << "ODM prepXfer: DRAM<->DRAM op=" << (operation == NIXL_READ ? "READ" : "WRITE")
                   << " -> "
                   << (req->direction == ODM_DIR_TO_GPU ? "READ (device->host)" :
                                                          "WRITE (host->device)")
                   << " segments=" << n;
    } else {
        ...
        NIXL_DEBUG << "ODM prepXfer: VRAM<->DRAM op=" << (operation == NIXL_READ ? "READ" : "WRITE")
                   << " -> "
                   << (req->direction == ODM_DIR_TO_GPU ? "READ_FD (device->VRAM)" :
...
static void
warnIfKernelOdmUringDisabled() {
    std::ifstream param("/sys/module/mrvl_cxl_pcie/parameters/odm_use_io_uring");
    if (!param) {
        return;
    }
    int val = 0;
    ...
    if (val == 0) {
        NIXL_WARN << "ODM: kernel module param odm_use_io_uring=0; io_uring uring_cmd "
                     "requires: echo 1 > /sys/module/mrvl_cxl_pcie/parameters/odm_use_io_uring";
    }
}
...
nixlOdmEngine::postOdmDmabufUring(nixlOdmBackendReqH *req) const {
    ...
    const int uring_op =
        (req->direction == ODM_DIR_TO_GPU) ? ODM_URING_CMD_READ_FD : ODM_URING_CMD_WRITE_FD;
...
nixlOdmEngine::postOdmHostVa(nixlOdmBackendReqH *req) const {
     ...
     */
    const unsigned int ioctl_cmd = (req->direction == ODM_DIR_TO_GPU) ?
        MRVL_CXL_DMA_READ_COMMAND :
        MRVL_CXL_DMA_WRITE_COMMAND;
    const uint32_t xfer_type = (req->direction == ODM_DIR_TO_GPU) ? ODM_XTYPE_OUTBOUND :
                                                                    ODM_XTYPE_INBOUND;
    const uint16_t req_qid = nextQid();
